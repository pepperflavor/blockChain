...
new RegExp(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/);
 ...
 function check() {
    ...
    console.log(container[0].match(nameReg)); //string이랑 배열이 값비교가 당연히 안되지^^
    console.log(container[0]);
    // 값 검증하기 시작
    // if (container[0] == container[0].match(nameReg)) {
    //     // 이름검증
    //     alert("이름을 잘못 입력하셨습니다.");
    // } else if (container[1] ==  container[1].match(idReg)) {
    //     // id 검증
    //     alert("ID가 잘못된 형식입니다.");
    // } else if  (container[2] ==  container[2].match(telReg)) {
    //     // 연락처 검증
    //     alert("연락처가 잘못된 형식입니다.");
    // }else if(container[3] == container[3].match(mailReg)){
    //     // 메일검증
    //     alert("이메일이 잘못된 형식입니다.");
    // }else if(container[4] == container[4].match(pwdReg)){
